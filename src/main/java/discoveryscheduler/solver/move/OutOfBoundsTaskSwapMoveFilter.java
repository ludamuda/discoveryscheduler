        ...
        for(Object assignedValue : assignedValues){
        	if(assignedValue instanceof Timestamp){
				...
				if(leftTask.equals(rightTask)){
		    	 ...
		    	 else if((!(start.getDay().equals(rightTask.getGroup().getGroupTimestampList().get(start.getTimestampIndex()-rightTask.getGroup().getGroupTimestampList().get(0).getTimestampIndex()+rightTask.getActivity().getLength()).getDay())))
		    			|| (!(start.getDay().equals(leftTask.getGroup().getGroupTimestampList().get(start.getTimestampIndex()-leftTask.getGroup().getGroupTimestampList().get(0).getTimestampIndex()+leftTask.getActivity().getLength()).getDay())))){
		    		return false;
		    	}
		    	*/
			} 
